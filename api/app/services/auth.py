...
Access-token validation lives in :mod:`app.core.security`; this use case maps a
set of validated claims to a local user, creating it (and its settings row) on
first login and owning the transaction.
"""
...
async def get_or_create_user(db: AsyncSession, claims: dict[str, Any]) -> models.User:
    ...
    user = await users_repo.get_by_sub(db, sub)
